...
export const PhotoGallery = ({
  ...
  // 2-second automatic slide back timer
  useEffect(() => {
    if (hoveredId === null) return;
    
    const timer = setTimeout(() => {
      setHoveredId(null);
    }, 2000); // Reset hover focus after exactly 2 seconds
  ...
  // Photo configurations - horizontal layout on desktop, stacked deck layout on mobile
  const photos = [
    {
      id: 1,
      order: 0,
  ...
  return (
    ...
    >
      <div className="absolute inset-0 max-md:hidden top-[20px] -z-10 h-[300px] w-full bg-transparent bg-[linear-gradient(to_right,#e2e8f0_1px,transparent_1px),linear-gradient(to_bottom,#e2e8f0_1px,transparent_1px)] bg-[size:3rem_3rem] opacity-20 [mask-image:radial-gradient(ellipse_80%_50%_at_50%_0%,#000_70%,transparent_110%)] pointer-events-none"></div>
      ...
      <div className="relative mb-4 h-[300px] md:h-[380px] w-full flex items-center justify-center overflow-visible">
        ...
        >
          <div className="relative flex w-full justify-center h-[240px] md:h-[280px] overflow-visible">
            ...
            {/* ── Layer 1: Static Invisible Hover Triggers (Optimized & Responsive for Mobile/Desktop) ── */}
            {isLoaded && photos.map((photo) => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    "absolute cursor-pointer z-50 bg-transparent",
                    isMobile ? "w-[120px] h-[160px]" : "w-[180px] h-[240px]"
                  )}
                  style={{
                    left: "50%",
                    top: "50%",
                    transform: `translate(-50%, -50%) translate(${currentX}, ${currentY})`,
                  }}
            ...
            {/* ── Layer 2: Beautiful Animated Visual Cards ── */}
            <div className={cn(
              "relative overflow-visible",
              isMobile ? "h-[160px] w-[120px]" : "h-[240px] w-[240px]"
            )}>
...
export const Photo = ({
  ...
  return (
    <div
      ...
      className={cn(
        className,
        "relative mx-auto shrink-0 select-none pointer-events-none"
      )}
      draggable={false}
    >
      <div className={cn(
        ...
        isFocused 
          ? "border border-pink-300 bg-slate-900 shadow-[0_18px_45px_rgba(169,6,201,0.22)]" 
          : "border border-slate-200/50 shadow-[0_10px_25px_rgba(0,0,0,0.05)] bg-slate-950"
      )}>
        <img
          className={cn(
            "object-cover h-full w-full pointer-events-none select-none transition-all duration-500",
            isMobile ? "rounded-[1.2rem]" : "rounded-[2rem]",
            isFocused ? "brightness-100 scale-102" : "brightness-75 scale-100"
          )}
          src={src}
          alt={alt}
          draggable={false}
        ...
        {/* Real Work Typography Details */}
        <div className={cn(
          "absolute inset-x-0 bottom-0 flex flex-col justify-end text-white select-none pointer-events-none",
          isMobile ? "p-3" : "p-5"
        )}>
          <span className={cn(
            "font-bold text-[#f016da] bg-[#f016da]/10 border border-[#f016da]/25 rounded-full w-fit mb-1 uppercase tracking-wider",
            isMobile ? "text-[6.5px] px-1.5 py-0.5" : "text-[9px] px-2 py-0.5"
          )}>
            {tag}
          </span>
          <h4 className={cn(
            "font-bold leading-tight tracking-tight text-white select-none",
